...
- **Notes:** Updated the color palette in `src/constants/theme.ts` (added `accent`, `mint`, `peach` tokens; light and dark modes both use the same pastel palette — no more black background). Home screen (`src/app/index.tsx`) originally had its own "Productivity Pet" header/tagline (later moved into the global `AppHeader`, see FEATURE-013), a pastel pet card, a new static "Egg Stage — 0 / 5 tasks to hatch" progress card (`src/components/pet-progress.tsx`), and restyled task rows with a custom checkbox and an accent-colored Add button. The progress card was a visual placeholder only at first — not wired to the real task count, no growth/leveling logic — until FEATURE-023 redesigned it with real 5-stage progress. The plain pet card was later replaced by the living-room scene in FEATURE-007. Multiple rooms, pet movement/animation, and shop/accounts are still future work.
...
- **Notes:** `src/components/pet-placeholder.tsx` now renders `assets/images/pets/egg_nest.png` (1536x1024, transparent background) via `expo-image` with `contentFit="contain"` and an `aspectRatio` matching the asset's real pixel ratio, sized as a responsive percentage width (`46%`, clamped between 150–260px) so it scales with the room rather than using a fixed pixel size. It's wrapped in a `Pressable`; a manual double-tap check (two taps within 300ms, via a timestamp ref — no gesture library needed) triggers the hatched state. Once hatched, the dimmed egg image + pet overlay stay that way permanently (in memory — resets on page refresh; no persistence yet). See FEATURE-011 for the current hatched-state visual and behavior, which was refined after this was first built. `src/components/pet-room.tsx`'s overlay pointer events were changed from `none` to `box-none` so taps can reach the egg while the rest of the overlay stays passthrough.
  **The whole double-tap mechanism described above was removed in FEATURE-024** — growth is now automatic and driven by Pet Progress, not a tap gesture. `pet-room.tsx`'s `pointerEvents="box-none"` was left as-is (harmless now that nothing claims the touch) since changing it wasn't part of that request.
...
- **Notes:** In `src/components/pet-placeholder.tsx`, `hasHatched` is now a one-way state — double-tapping sets it true and it is never reset (no timeout reverts it), so the pet stays hatched until the page is refreshed (no persistence yet, per instructions). Once hatched, further taps are ignored (`handlePress` returns early). First implemented as a "🐱" emoji overlay on the dimmed egg art (no kitten asset yet); replaced by real illustrated art in FEATURE-012. The "Crack!" message bubble is a separate, still-temporary flash (`showCrackMessage`, reverts after 1.8s) so it doesn't block real task-completion messages (FEATURE-005) from showing again after hatching.
  **Removed in FEATURE-024** along with the rest of the tap-to-hatch mechanism — `hasHatched` no longer exists; the pet's stage is derived from completed tasks instead.
...
- **Notes:** `src/components/pet-placeholder.tsx` now swaps its `expo-image` source between `assets/images/pets/egg_nest.png` (before hatching) and `assets/images/pets/kitten_nest.png` (after) based on `hasHatched`, both with `contentFit="contain"`. The two source images have different pixel aspect ratios (egg: 1536x1024; kitten: 1381x1139), so the container's `aspectRatio` now switches along with the source to match whichever image is showing — this is what prevents stretching for both states. The kitten asset needed a re-export from the person providing it: the first version had no alpha channel and baked a visible gray checkerboard into the pixels instead of real transparency; the corrected version was verified (alpha channel present, corners fully transparent) before wiring it in. No animation or growth-stage logic beyond this one-time swap yet.
  `kitten_nest.png` itself is still used — it's now the dedicated Hatchling-stage image (per explicit instruction in FEATURE-024 to keep it) — only the tap-driven swap mechanism was removed.
...
- **Notes:** The trigger logic lives in `src/app/index.tsx`'s `toggleTask`, not as a generic "watch the percentage" effect — this is deliberate. After computing the updated tasks array, it checks `completed && updatedTasks.length > 0 && updatedTasks.every(t => t.completed)`: only true when (a) this specific action just *set* a task to completed (not uncompleted), and (b) that action happened to make every task complete. This precisely satisfies all the required behavior for free:
  ...
  - Adding tasks, completing/uncompleting, persistence, the tracker/progress bar, categories, icons, Mark as Important, the important star, the three-dot menu, deleting, header, living room, Pet Progress, and hatching were all left untouched. The pet reaction for unchecking a completed task was explicitly not added at the time — added separately afterward (a temporary "Aww, not done yet? You got this! 💕" speech-bubble message on uncheck, using the same timeout-ref pattern as the other pet messages).

### [FEATURE-023] Pet Progress redesign: real 5-stage growth tracker

- **Status:** Done
- **Description:** Replace the static "Egg Stage — 0/5 tasks to hatch" placeholder with a real progress tracker matching the reference: "Pet Progress" / "Level X" header, a pastel progress bar, "X / 10 tasks to next stage" text, and an Egg → Hatchling → Baby → Young → Adult stage row (10 completed tasks per stage). Progress is driven by actual completed tasks — checking a task raises it, unchecking lowers it — and survives a refresh. Pet artwork in the living room itself is untouched; this is only the Pet Progress card.
- **Notes:** `src/components/pet-progress.tsx` now takes a `completedTaskCount: number` prop (passed from `src/app/index.tsx` as `tasks.filter(t => t.completed).length`, computed fresh on every render — no new state or persistence). Stage/level/in-stage-progress are all derived from that single number: `stageIndex = min(floor(completedTaskCount / 10), 4)`, `level = stageIndex + 1`, and the "X / 10" count resets to 0 right as you cross into a new stage. Once at the last stage (Adult), the bar shows full and the text switches to "Max stage reached! 🎉" instead of a nonsensical "10/10 tasks to a stage that doesn't exist" — a small addition beyond the literal spec, needed for the max-stage case to read correctly.
  - Because everything is derived live from `completedTaskCount`, unchecking a task (which lowers that count) automatically lowers the stage/level/bar too — including dropping back a whole stage if it crosses a 10-task boundary — with no separate "regression" logic needed. Progress persists across refresh for the same reason: it's derived from the `tasks` array already persisted via `AsyncStorage` (FEATURE-016), not stored as its own separate number.
  - The 5 stages originally used simple emoji placeholders (🥚 🐣 🐱 😺 🐈), each in a circular pastel chip; the current stage gets an `accent`-colored ring around its chip. This was intentionally not the illustrated pet/room artwork at the time — per instructions, the living-room pet art was untouched in this step. The emoji were replaced with real illustrated icons shortly after (`src/components/pet-stage-icon.tsx`, cropped from `assets/images/pets/pet_stages.png` — same sprite-crop technique as `TaskIcon`), and the living-room pet itself later became stage-aware too — see FEATURE-024.
  - The progress bar reuses the same numeric `flex`-ratio fill technique as `TaskCard`'s completion tracker (FEATURE-021), for the same `DimensionValue`-typing reason.
  - Verified the stage/level math against a range of completed-task counts (0, 3, 9, 10, 11, 25, 39, 40, 45) before wiring it in — correctly transitions at every 10-task boundary and caps cleanly at Adult regardless of how far past 40.
  - Task list, persistence, categories/icons, Mark as Important, the three-dot menu, the completion tracker/celebration, header, living room, and hatching were all left untouched.

### [FEATURE-024] Automatic pet growth tied to Pet Progress (double-tap hatching removed)

- **Status:** Done
- **Description:** Remove the double-tap-to-hatch gesture entirely. The pet shown in the living room now grows automatically through Egg → Hatchling → Baby → Young → Adult as completed tasks cross each Pet Progress stage boundary, always matching what Pet Progress shows. Egg keeps `egg_nest.png`; Hatchling keeps the existing kitten-in-shell `kitten_nest.png` (explicitly not replaced with the Hatchling pose from the new sprite); Baby/Young/Adult use new art cropped from `assets/images/pets/pet_growth_stages.png`.
- **Notes:** New `src/utils/pet-stage.ts` is the single source of truth for the stage formula — `PetStage`, `PET_STAGES`, `TASKS_PER_STAGE` (10), `getPetStageIndex(completedTaskCount)`, `getPetStage(completedTaskCount)`. Both `PetProgress` (FEATURE-023) and the living-room pet now derive their stage from this same shared function instead of each having their own copy, so they can never disagree. `src/components/pet-stage-icon.tsx` was updated to import `PetStage` from here rather than declaring its own local copy of the type.
  - `src/app/index.tsx` computes `petStage = getPetStage(completedTaskCount)` (the same `completedTaskCount` already used by `PetProgress`) and passes it down through `PetRoom` to `PetPlaceholder`. Since this is derived fresh from `tasks` on every render — no new state — the pet updates immediately when a task is completed or unchecked, and survives a refresh for the same reason the rest of this data already does (`tasks` is the thing persisted via `AsyncStorage`, not the derived stage).
  - `src/components/pet-placeholder.tsx` was completely rewritten: all double-tap state (`hasHatched`, `showCrackMessage`, the tap-timestamp ref, the "Crack!" message, the dimmed-egg + 🐱-emoji-overlay hatched look) is gone. It now takes a `stage: PetStage` prop and a small `getStageImage(stage)` switch picks the right image + real aspect ratio for that stage — Egg/Hatchling use their existing full illustrations (1536x1024 / 1381x1139), Baby/Young/Adult use the new pre-cropped 540x540 square files. No `Pressable` remains; the pet is no longer tappable.
  - **Cropping `pet_growth_stages.png` (2172x724, contains Hatchling/Baby/Young/Adult poses — Hatchling intentionally unused per instructions):** rather than crop at runtime (which would need a measured pixel size — this component is responsively sized via percentage width, unlike the small fixed-size `TaskIcon`/`PetStageIcon`), the Baby/Young/Adult regions were located by analyzing the source image's pixel opacity (row/column density scans to separate art from label pills, then per-pose bounding boxes) and pre-cropped into three standalone files — `pet_baby.png`, `pet_young.png`, `pet_adult.png` — verified visually before wiring in, matching the "one full image per stage" pattern Egg/Hatchling already used.
  - Task list, persistence, categories/icons, Mark as Important, the three-dot menu, the completion tracker/celebration, header, and Pet Progress's own UI were all left untouched.

### [FEATURE-025] Today's Mood card (static placeholder)

- **Status:** Done
- **Description:** Add a "Today's Mood" pastel card matching the rest of the dashboard, showing the pet's mood as "Happy 😊". Placed underneath the Today's Tasks card, in the left column on the wide/desktop layout. The mood does not change automatically yet — that's planned as a separate next step.
- **Notes:** New `src/components/today-mood.tsx` (`TodayMood`) — a simple `ThemedView type="backgroundElement"` card (same tone as `TaskCard`, for visual continuity in the stacked left column) with a "Today's Mood" title (same style as `TaskCard`'s title) and a mood row (😊 emoji + "Happy" text). No props, no state — fully static for now.
  - `src/app/index.tsx`: on the wide/two-column layout, `TodayMood` renders inside `leftColumn` directly below `TaskCard` (a `gap` was added to `leftColumn` so the two cards have spacing between them); `PetRoom` stays alone in the right column, and `PetProgress` is unchanged (still full-width below the row, not part of the left column). On the narrow/mobile stacked layout, `TodayMood` was added right after `TaskCard` to keep the same relative ordering.
  - This emoji-based static version was replaced almost immediately by the illustrated, selectable version in FEATURE-026 — kept "Done" here (not "In Progress") since the placeholder itself was fully finished before being superseded, and the layout/placement decisions above still stand unchanged.

### [FEATURE-026] Selectable illustrated moods with persistence

- **Status:** Done
- **Description:** Redesign Today's Mood to match the reference: real illustrated mood icons (not emoji) for Happy, Calm, Focused, Stressed, Tired, and Sick; clicking one selects it; the selected mood shows larger next to a cute encouraging message; the selection persists across refresh. Moods are still chosen manually — not yet connected to task activity (planned separately, per instructions).
- **Notes:** New `src/components/mood-icon.tsx` (`MoodIcon`, plus the `Mood` type and `MOODS` list) crops the matching card out of the new `assets/images/moods/pet_moods.png` (1945x809 — 6 rounded-card mood illustrations, each with its own baked-in pastel background, laid out left to right, with text-label pills below that aren't used since `TodayMood` renders its own labels). Same sprite-crop technique as `TaskIcon`/`PetStageIcon`, adapted for a non-square (portrait rounded-rect) crop to match these cards' actual shape; crop boundaries were located by analyzing the source's pixel opacity (column-gap detection, since each card is cleanly separated) and verified against test crops for 3 of the 6 moods before wiring in.
  - `src/components/today-mood.tsx` was rewritten: a `mood: Mood` state (default `'Happy'`) drives a larger `MoodIcon` (size 72) shown beside the mood name and a per-mood encouraging message (`MOOD_MESSAGES`, one cute line per mood, all manually written since none were specified). Below that, all 6 moods render as smaller (`size 36`) tappable `MoodIcon`s in a wrapping row; the selected one gets an `accent`-colored border frame.
  - Persistence follows the same pattern already used for tasks (`src/app/index.tsx`) and now mirrored here: `AsyncStorage` under its own key (`@ProductivityPet:mood`), loaded once on mount with an `isHydrated` guard before the save effect can run, so the default `'Happy'` can never overwrite a previously-saved mood before the load finishes. This is a second, independent instance of that pattern — not shared state with the tasks list.
  - Everything else (Today's Tasks, Pet Progress, the living room/pet growth, header, navigation) was left untouched.
  - **The mood state (and its single-fixed-message-per-mood) described above moved up into `src/app/index.tsx` in FEATURE-027**, so `TodayMood` became a controlled component and the mood could also drive the pet's speech bubble; `today-mood.tsx`'s own local `MOOD_MESSAGES` record was replaced by a shared multi-message pool.

### [FEATURE-027] Mood connected to the pet's speech bubble, with several messages per mood

- **Status:** Done
- **Description:** Connect the selected mood to the pet's speech bubble in the living room. Each mood has several cute messages; one is chosen at random whenever the mood changes, and the message occasionally (not constantly/rapidly) re-rolls to a different one from the same pool while idle. The selected mood still persists across refresh.
- **Notes:** New `src/utils/mood.ts` centralizes `Mood`, `MOODS`, `MOOD_MESSAGES` (4 hand-written messages per mood, matching the ones specified — light emoji were added to a few that appeared to have lost theirs in transcription, for visual consistency with the rest of the app) and `getRandomMoodMessage(mood, exclude?)`, which avoids repeating the immediately-previous message when the pool allows it. `src/components/mood-icon.tsx` now imports `Mood` from here instead of declaring it locally (mirrors how `pet-stage.ts` centralized `PetStage` in FEATURE-024).
  - **Mood state moved from `TodayMood` up into `src/app/index.tsx`** (mirroring how `tasks` already works: hydrate once from `AsyncStorage` under `@ProductivityPet:mood`, save on change after hydration) since two things now need it — the mood-selector card and the pet's speech bubble. `TodayMood` became a controlled component (`mood`, `message`, `onSelectMood` props) instead of owning its own state; its own inline "beside the icon" message is now the exact same ambient message the pet is showing, kept in sync from one shared source rather than two separate texts that could drift.
  - **Message priority system:** the speech bubble shown to `PetRoom` is `temporaryMessage ?? moodMessage` — reactive task messages (the existing completion/uncheck messages, renamed from `petMessage` to `temporaryMessage`) take priority while active; otherwise the ambient mood message shows. A `useEffect` keyed on `mood` immediately rolls a fresh random message on mood change and starts a `setInterval` (`MOOD_MESSAGE_REFRESH_INTERVAL_MS`, 60s) that occasionally re-rolls a *different* message from the same pool — this is the "occasionally... when appropriate" behavior: it always runs, but is only visibly noticeable when no temporary message is currently covering it.
  - **One small necessary behavior change to make this work:** the task-completion message ("Yippee!" etc.) previously had no auto-revert timer at all — it would sit in the bubble forever until the next task was toggled. For the ambient mood message to ever be visible after the first task completion, completion messages now auto-revert after the same 3s duration the uncheck message already used (`TEMPORARY_MESSAGE_DURATION_MS`, shared by both branches now via one timeout ref). This wasn't explicitly requested but follows directly from "connect mood to the speech bubble" — flagged here in case the timing should be tuned.
  - Verified the message pools (4 per mood, 24 total) and the no-immediate-repeat behavior (200 consecutive re-rolls, zero repeats) with a script before wiring in.
  - Task list, persistence, categories/icons, Mark as Important, the three-dot menu, the completion tracker/celebration, header, Pet Progress, and pet growth/stage logic were all left untouched.

### [FEATURE-028] Rooms card (visual layout only)

- **Status:** Done
- **Description:** Add a "Rooms" pastel card to the dashboard: 6 room previews (Living Room, Study Room, Gym, Bedroom, Bathroom, Kitchen) in a 3-column × 2-row grid, Living Room shown as selected/highlighted, "Rooms" at top-left and "View All →" at top-right. On the wide/desktop layout it sits in the right column, below the living room (filling the space beside Today's Mood, which is in the left column at the same row position — the two columns are now symmetric, two cards stacked in each). On narrow/mobile it's stacked at the very end. Visual layout only — room cards are not pressable and switching rooms doesn't work yet, per instructions.
- **Notes:** New `src/components/rooms-card.tsx` (`RoomsCard`). Living Room's preview originally used a small cropped `contentFit="cover"` thumbnail of the real `assets/images/rooms/living-room.png` art (it's the one room that actually exists); the other 5 rooms didn't have art yet, so they used simple emoji placeholders (📚 🏋️ 🛏️ 🛁 🍳) — the same "emoji now, real art later" approach this project has used for every other not-yet-illustrated feature. **All 6 were replaced with real illustrated previews in FEATURE-029.** The selected room (Living Room) gets an `accent`-colored border; the rest use the app's existing pastel tokens (`backgroundElement` border) — no new colors. The 3-per-row grid uses `flexWrap` with each item at `width: '31%'`, which fits comfortably within both the left (min 320px) and right (min 380px) column widths.
  - `src/app/index.tsx`: `rightColumn` now renders `PetRoom` then `RoomsCard` (gained a `gap`, mirroring the `gap` `leftColumn` already had for its two stacked cards); the narrow/mobile stack gained `RoomsCard` at the end.
  - Today's Tasks, Pet Progress, Today's Mood, the pet's speech bubble/mood system, header, navigation, and category/task logic were all left completely untouched.

### [FEATURE-029] Illustrated room previews replace emoji placeholders

- **Status:** Done
- **Description:** Replace the Rooms card's temporary emoji placeholders (and the ad-hoc `living-room.png` crop used for Living Room) with the 6 matching illustrated room previews from the new `assets/images/rooms/room_previews.png`. Same 3×2 layout, Living Room still highlighted. Visual only — rooms remain non-functional.
- **Notes:** `room_previews.png` (1536x1024, transparent background — the "blurred" area around the cards is real low-alpha transparency, not opaque blur, confirmed by sampling pixel alpha before relying on it) is a clean 3×2 grid where each cell is a single fused card: illustration + a baked-in pastel label pill (e.g. "Living Room") as one continuous opaque shape, unlike the earlier task/mood/pet-stage sprites where the label sat separately below the art with a gap. Column and row boundaries were found the same way as those (alpha-density scans, threshold raised to catch only the fully-opaque card regions and ignore the transparent blur), giving 6 per-cell bounding boxes (~444-454 × 454-470px, i.e. very close to square).
  - Because each cell's label is fused into the art rather than separate, and because these grid tiles are responsively sized (percentage width, so there's no known pixel size for a runtime sprite-crop the way `TaskIcon`/`PetStageIcon`/`MoodIcon` do it), all 6 were pre-cropped into standalone files — `room_preview_living.png`, `_study.png`, `_gym.png`, `_bedroom.png`, `_bathroom.png`, `_kitchen.png` — same approach as the pet growth stages (FEATURE-024), and verified visually (2 of 6) before wiring in.
  - Since each image now already shows its own room name, `RoomsCard`'s separate `ThemedText` label under each icon was removed — keeping it would have duplicated the name. `contentFit` changed from `cover` to `contain` so each card's full illustration (label pill included) is always fully visible rather than possibly cropped.
  - **Living Room's preview changed source**, not just format: it now uses the same purpose-built, stylistically-consistent card art as the other 5 (from `room_previews.png`) instead of a crop of the large landscape `living-room.png` scene used elsewhere in the app. This wasn't strictly requested (the instructions specifically called out replacing "emoji placeholders," and Living Room wasn't one) but was a small deliberate choice for visual consistency — all 6 grid cards now share identical framing/style rather than mixing one oddly-cropped photo-style scene with 5 purpose-made cards.
  - Everything else — layout, grid structure, Living Room's selected/highlighted state, header text, and all other dashboard sections — is unchanged.

### [FEATURE-030] Interactive room cards open a per-room detail page

- **Status:** Done
- **Description:** Clicking any of the 6 room preview cards on the dashboard's Rooms card now opens a dedicated page for that room: the header/navigation (already global, via `_layout.tsx`), the room's artwork shown prominently, the room name, a short cute description, and a button back to the Rooms page. The Rooms card itself keeps its exact existing look — only click behavior was added. Individual rooms otherwise remain non-functional (no room-specific actions beyond viewing).
- **Notes:** New `src/app/rooms/[room].tsx`, an Expo Router dynamic route (`/rooms/:room`), coexisting alongside the existing flat `src/app/rooms.tsx` (the "Rooms" tab / `ComingSoonScreen`, itself untouched) — file-based routing supports a flat file and a same-named directory as siblings, confirmed by a full clean rebuild (`expo export -p web -c`) producing both `/rooms` and `/rooms/[room]` successfully.
  - New `src/utils/rooms.ts` centralizes room detail data (`RoomSlug`, `RoomInfo`, `ROOMS`, `ROOM_ORDER`, `isRoomSlug`) — name, a hand-written one-line cute description per room, the artwork to show, and its real aspect ratio. At this stage all 6 rooms (including Living Room) had a `/rooms/[room]` detail page; **FEATURE-031 removed Living Room from this map entirely** — its card now goes straight back to Home instead.
  - New `src/components/room-detail-screen.tsx` (`RoomDetailScreen`) is the shared screen shell used by the dynamic route for all rooms: art, room name (`subtitle` type), description, and a pastel `accent`-colored "← Back to Rooms" button that navigates to `/rooms` — matching the pastel/cute styling used everywhere else in the app. The art was originally capped at 420px wide; **FEATURE-031 made it full-width/prominent** and added a `ScrollView` for smaller screens.
  - `src/components/rooms-card.tsx`: each of the 6 preview `View`s is now wrapped in a `Pressable` navigating to `{ pathname: '/rooms/[room]', params: { room: room.slug } }` (Expo Router's typed routes require this object form for dynamic segments — a plain interpolated string path like `` `/rooms/${slug}` `` doesn't type-check). Fixing this required moving the grid item's `width: '31%'` from the inner `View` onto the new outer `Pressable` (a percentage width needs to resolve against its *immediate* parent — leaving it on the inner `View` would have resolved against the now-intervening `Pressable`, which has no explicit width, the same class of bug hit earlier with `pet-placeholder.tsx`). Visually the cards are pixel-identical to before.
  - Task list, progress tracking, Today's Mood, Pet Progress, the living room/pet growth, header, and the Rooms card's own visual design/artwork were all left completely unchanged.

### [FEATURE-031] Real functional room pages with full-size backgrounds

- **Status:** Done
- **Description:** Living Room's preview card now navigates back to Home (the existing living room, unchanged) instead of a separate detail page. The other 5 rooms (Study Room, Gym, Bedroom, Bathroom, Kitchen) each open a page showing their own new full-size illustrated background, large and prominent — the same visual treatment the living room gets on Home. No pet/cat art on these pages yet, backgrounds only, per instructions.
- **Notes:** Added 5 new background images (`assets/images/rooms/backgrounds/Study_Room.png`, `Gym_Room.png`, `Bedroom.png`, `Bath_Room.png`, `Kitchen_Room.png` — ~1672×941, ratio ≈1.777, opaque RGB, no alpha needed since these are full-bleed scene backgrounds, not sprites). Inspected each file's dimensions before wiring in.
  - `src/utils/rooms.ts`: `RoomSlug`/`ROOMS`/`ROOM_ORDER` narrowed from 6 rooms to the 5 functional sub-rooms — Living Room is no longer part of this map at all, since it's no longer a `/rooms/[room]` destination. Each remaining room's `image`/`aspectRatio` now points at its new background file instead of the small `room_preview_*.png` card used previously.
  - `src/components/rooms-card.tsx`: `RoomPreview.slug` is now `RoomSlug | null` — `null` means "not a detail page." A small `openRoom(slug)` helper does `router.push('/')` for Living Room (`slug === null`) or the existing `/rooms/[room]` push otherwise. The 6 preview cards' own art/layout/style are completely unchanged — only which destination each one's existing `Pressable` navigates to.
  - `src/components/room-detail-screen.tsx`: the art display changed from a small `maxWidth: 420` thumbnail to full-width (`width: '100%'`, aspect-ratio locked, no cap beyond the page's own `MaxContentWidth`) — matching how `PetRoom` shows the living-room background on Home ("large and prominent... similar to how the Living Room artwork is displayed"). Also wrapped the screen content in a `ScrollView` (mirroring the pattern already used on Home and `TaskCard`), since a full-width ~1.78-ratio image plus name/description/button can now exceed a small phone's viewport height — this is what keeps "responsive... for desktop and mobile" true now that the art is much bigger than before.
